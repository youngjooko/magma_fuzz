...
#define FLIP_BIT(_ar, _b)                   \
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    afl->cur_offset = afl->stage_cur_byte;
...
havoc_stage:
  ...
  for (idx =0; idx<32; idx++) afl->offsets[idx] = -1;
  idx = 0;
    ...
    for (i = 0; i < use_stacking; ++i) {
      ...
      switch ((r = rand_below(afl, r_max))) {
...
#endif
          u32 r_idx = rand_below(afl, temp_len <<3);
          if (idx<32){
            afl->offsets[idx] = r_idx;
            idx++;
          }
          FLIP_BIT(out_buf, r_idx);
          break;
        ...
        case 4 ... 7: {
          ...
          u32 r_idx = rand_below(afl, temp_len);
          if (idx<32){
            afl->offsets[idx] = r_idx;
            idx++;
          }
        ...
        case 8 ... 9: {
          ...
          u32 r_idx = rand_below(afl, temp_len -1);
          if (idx<32){
            afl->offsets[idx] = r_idx;
            idx++;
          }
        ...
        case 16 ... 19: {
          ...
          out_buf[r_idx] -= 1 + rand_below(afl, ARITH_MAX);
          break;
        ...
        case 40 ... 43: {
          ...
          out_buf[r_idx] ^= 1 + rand_below(afl, 255);
          break;
...
/**************
   * Memory intensive mutation
   * ************/

  /*MFuzz: mutation*/
  if (afl->queue_cur->is_mem)
  {

    debug_write(afl,"afl mutation start");
    u32 i;
    for (i=0; i <32; i++){
      if (afl->queue_cur->offsets[i] != -1)
      {


        debug_write(afl,"afl mutation insert");
        /* Insert */
        u32 j;
        u32 cur_len = afl->queue_cur->len;
        u32 mutation_to = afl->queue_cur->offsets[i];
        if (mutation_to > cur_len) continue;

        for(j=0; j <20; j++){
          u32 clone_len = 1<<j;
          afl->debug_insert++;
          u8 *new_buf =
            afl_realloc(AFL_BUF_PARAM(out_scratch), cur_len + clone_len);
          if (unlikely(!new_buf)) { PFATAL("alloc"); }

          /* Head */

          memcpy(new_buf, out_buf, mutation_to);

          /* Inserted part */

          memset(new_buf + mutation_to,
              rand_below(afl, 2) ? rand_below(afl, 256)
              : out_buf[rand_below(afl, cur_len)],
              clone_len);


          /* Tail */
          memcpy(new_buf + mutation_to + clone_len, out_buf + mutation_to,
              cur_len - mutation_to);

          out_buf = new_buf;
          afl_swap_bufs(AFL_BUF_PARAM(out), AFL_BUF_PARAM(out_scratch));

          if (common_fuzz_stuff(afl, out_buf, cur_len + clone_len)) { goto abandon_entry; }

          out_buf = afl_realloc(AFL_BUF_PARAM(out), cur_len);
          if (unlikely(!out_buf)) { PFATAL("alloc"); }
        ...
        /*Delete*/
        u32 remain_len = cur_len - mutation_to;
        for (j = 0; j < 20 ;j++){
          u32 del_len = 1<<j;
          if (remain_len < del_len) break;

          afl->debug_delete++;
          memmove(out_buf + mutation_to, out_buf + mutation_to + del_len,
              cur_len - mutation_to - del_len);

          if (common_fuzz_stuff(afl, out_buf, cur_len - del_len)) { goto abandon_entry; }
          out_buf = afl_realloc(AFL_BUF_PARAM(out), cur_len);
          if (unlikely(!out_buf)) { PFATAL("alloc"); }
          memcpy(out_buf, in_buf, cur_len);
    ...
    debug_write(afl,"afl mutation end");
  }
