...
void write_memory_coverage(afl_state_t *afl){

  u32 i;

  u8 fn[PATH_MAX];
  snprintf(fn, PATH_MAX, "%s/memory_coverage", afl->out_dir);
  FILE *f = create_ffile(fn);

  for (i=0; i< afl->queued_items; i++){
    struct queue_entry *q = afl->queue_buf[i];
    fprintf(f,"%u %u | %u %u\n", q->mem, q->avg_mem,q->mem2, q->avg_mem2);
...
void write_memory_coverage_debug(afl_state_t *afl, u32 mem, u32 mem2, u32 avg_mem, u32 avg_mem2){


  u8 fn[PATH_MAX];
  snprintf(fn, PATH_MAX, "%s/memory_coverage_debug", afl->out_dir);
  FILE *f = fopen(fn,"a+");

  fprintf(f,"%u %u | %u %u\n", mem, avg_mem,mem2, avg_mem2);
...
void debug_write(afl_state_t *afl, char* str){


  u8 fn[PATH_MAX];
  snprintf(fn, PATH_MAX, "%s/debug_write", afl->out_dir);
  FILE *f = fopen(fn,"a+");
  ...
  fprintf(f,
      ...
      afl->fsrv.total_execs /
          ((double)(afl->prev_run_time + get_cur_time() - afl->start_time) /
           1000),
      afl->last_avg_execs_saved, afl->queued_path_items,afl->queued_mem_items, afl->queued_favored,
      ...
      (afl->unicorn_mode || afl->fsrv.qemu_mode || afl->fsrv.cs_mode ||
       ...
       afl->persistent_mode || afl->deferred_mode)
          ? ""
          : "default",
      afl->orig_cmdline,afl->debug_insert, afl->debug_delete);
...
#define SP20 SP10 SP10
  ...
  SAYF(bSTG bV bSTOP " corpus count : " cRST "%-5s " bSTG bV "\n",
       u_stringify_int(IB(0), afl->queued_path_items));
