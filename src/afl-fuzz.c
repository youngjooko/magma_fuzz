...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->fsrv.mem_bits = shmat((afl->shm).shm_mem_id, NULL, 0);
  afl->fsrv.mem2_bits = shmat((afl->shm).shm_mem2_id, NULL, 0);

  if (afl->fsrv.mem_bits == (void *)-1 || afl->fsrv.mem2_bits == (void *)-1)
    PFATAL("mem shmat() failed");
  ...
  if (afl->in_bitmap) {
    ...
    memset(afl->virgin_mem_bits, 255, MAP_SIZE);
    memset(afl->virgin_mem2_bits, 255, MAP_SIZE);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      afl->queue_cur->same_cov = 0;
      afl->queue_cur->new_mem = 0;
