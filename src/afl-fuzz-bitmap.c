...
u8 has_new_mem_bits(afl_state_t *afl, u32* mem, u32* mem2) {

#ifdef WORD_SIZE_64

  u64 *current_mem = (u64 *)afl->fsrv.mem_bits;
  u64 *virgin_mem  = (u64 *)afl->virgin_mem_bits;

  u64 *current_mem2 = (u64 *)afl->fsrv.mem2_bits;
  u64 *virgin_mem2  = (u64 *)afl->virgin_mem2_bits;
  ...
  u32 *current_mem = (u32 *)afl->fsrv.mem_bits;
  u32 *virgin_mem  = (u32 *)afl->virgin_mem_bits;

  u32 *current_mem2 = (u32 *)afl->fsrv.mem2_bits;
  u32 *virgin_mem2  = (u32 *)afl->virgin_mem2_bits;
  ...
  while (i--) {
    ...
    if (unlikely(*current_mem) && unlikely(*current_mem& *virgin_mem)){
      u8* cur_mem = (u8*)current_mem;
      u8* vir_mem = (u8*)virgin_mem;

        for (u32 j = 0; j < idx; j++){

        if ((cur_mem[j] && vir_mem[j]) > 0 ) *(mem) = *(mem) + 1;
      }

      *virgin_mem &= ~*current_mem;
      ret = 1;
    }

     if (unlikely(*current_mem2) && unlikely(*current_mem2 & *virgin_mem2)){

       u8* cur_mem2 = (u8*)current_mem2;
      u8* vir_mem2 = (u8*)virgin_mem2;

      for (u32 j = 0; j < idx; j++){
        if ((cur_mem2[j] && vir_mem2[j] ) > 0 ) *(mem2) = *(mem2) + 1;
      }

      *virgin_mem2 &= ~*current_mem2;
      ret = 1;
    }
    current_mem++;
    virgin_mem++;
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  u8  fn[PATH_MAX];
  u8 *queue_fn = "";
  u8  new_bits = 0, keeping = 0, res, classified = 0, is_timeout = 0, new_mem = 0;
  s32 fd;
  u64 cksum = 0;
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    new_mem= has_new_mem_bits(afl, &mem1, &mem2);
    ...
    afl->num_mem_op = (u64)count_bytes(afl,afl->fsrv.mem_bits) + (u64)count_bytes(afl, afl->fsrv.mem2_bits);

    afl->mem1 = mem1;
    afl->mem2 = mem2;
    afl->is_mem = new_mem;
    u32 avg_mem =0, avg_mem2 =0;
    if (new_mem){
      q->new_mem++;
    }
    ...
    if (mem1 > 0){
      q->t_mem++;
      q->total_mem += mem1;
      avg_mem = (q->total_mem)/(q->t_mem);
      if (mem1 < 0.8 * avg_mem) return 0;
    }
    if (mem2 >0){
      q->t_mem2++;
      q->total_mem2 += mem2;
      avg_mem2 = (q->total_mem2)/(q->t_mem2);
      if (mem2 < 0.3 * avg_mem2) return 0;
    }

    /*DEBUG*/
   if (new_mem){
      write_memory_coverage_debug(afl,mem1,mem2, avg_mem, avg_mem2);
    }
    classified = new_bits;
  ...
  save_to_queue:
    ...
    queue_fn =
        alloc_printf("%s/queue/id:%06u,mem,%02u,%s", afl->out_dir, afl->queued_items,new_mem,
    ...
    queue_fn =
        alloc_printf("%s/queue/id_%06u,mem,%02u", afl->out_dir, afl->queued_items,new_mem);
