...
void afl_shm_deinit(sharedmem_t *shm) {
  ...
  shmctl(shm->shm_mem_id, IPC_RMID, NULL);
  shmctl(shm->shm_mem2_id, IPC_RMID, NULL);
#endif
...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size,
  ...
  if (shm->shm_mem_id < 0) {
    ...
    PFATAL("shmget() mem failed, try running afl-system-config");
  ...
  shm_mem_str = alloc_printf("%d", shm->shm_mem_id);
  setenv(SHM_MEM_VAR, shm_mem_str, 1);
  ck_free(shm_mem_str);

  shm_mem2_str = alloc_printf("%d", shm->shm_mem2_id);
  setenv(SHM_MEM2_VAR, shm_mem2_str, 1);
  ck_free(shm_mem2_str);
