...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  q->total_mem2 = 0;
  q->is_mem = afl->is_mem;
  q->num_mem_op = afl->num_mem_op;


  q->avg_mem = (cur_q == NULL || cur_q->mem == 0)? 0 : cur_q->total_mem/cur_q->mem;
  q->avg_mem2 = (cur_q == NULL || cur_q->mem2== 0) ? 0 : cur_q->total_mem2/cur_q->mem2;
  ...
  if (q->is_mem){
    if (afl->stage_cur_byte == -1){
      memcpy(q->offsets,afl->offsets,32 *sizeof(u32));
    }
    else {
      q->offsets[0] = afl->cur_offset;
      q->offsets[1] = -1;
    }
  }
  ...
  if(q->is_mem) afl->queued_mem_items++;
  else afl->queued_path_items++;
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  }else if (q->num_mem_op > 0){
   u64 new_mem = q->mem + q->mem2;
   u64 rate =new_mem/q->num_mem_op;
