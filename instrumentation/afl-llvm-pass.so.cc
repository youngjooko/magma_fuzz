...
#endif
  std::map<std::string, std::vector<Value*>> affectedVars;
  std::map<std::string, std::vector<Value*>> affectedPtrOps;
  std::vector<std::string> stringFunc = {"strcpy", "strlen", "strcat"};

  int findAffected(Module &M);
  bool isPointer(Value *ptr);
  bool isIntrinsic(Intrinsic::ID ID);
  bool isStringFunc(std::string name);
...
bool AFLCoverage::isPointer(Value* ptr) {
  unsigned tid = 0;
  if (AllocaInst * ai = dyn_cast<AllocaInst>(ptr)){
    tid = ai->getOperand(0)->getType()->getTypeID();
  } else if (GetElementPtrInst * pi = dyn_cast<GetElementPtrInst>(ptr)){
    tid = pi->getSourceElementType()->getTypeID();
  }

  if( tid >= llvm::ArrayType::ArrayTyID ) return true;

  return false;
}

bool AFLCoverage::isStringFunc(std::string name){
  if (find(stringFunc.begin(), stringFunc.end(), name) != stringFunc.end())
    return true;

  return false;
}

bool AFLCoverage::isIntrinsic(Intrinsic::ID ID){
...
int AFLCoverage::findAffected(Module &M){
	...
	for (auto &F: M){
		...
		while (idx < used.size())
		{
			...
			if (StoreInst * inst = dyn_cast<StoreInst>(U)){
				...
				if (isPointer(ptr)) affectedVars[fname].push_back(U);
			} else if (LoadInst * inst = dyn_cast<LoadInst>(U)){
				Value *ptr = inst->getPointerOperand();
				if (isPointer(ptr)) affectedVars[fname].push_back(U);
			} else if (CallInst * inst = dyn_cast<CallInst>(U)){
				if(Function *f = dyn_cast<Function>(inst->getCalledOperand()->stripPointerCasts())){
					auto ID = f->getIntrinsicID();
					if(isIntrinsic(ID)) affectedVars[fname].push_back(U);
					else {
						std::string name = f->getName();
						if(isStringFunc(name)) affectedVars[fname].push_back(U);
					}
				}
			}


			if (GetElementPtrInst * pi = dyn_cast<GetElementPtrInst>(U)){
				Value * operand = pi->getOperand(0);
				if (find(used.begin(), used.end(), operand) == used.end()){
					used.push_back(operand);
				}
			}

			for (auto u: U->users()){
				if (find(used.begin(), used.end(), u) == used.end()) used.push_back(u);

				if (StoreInst *si = dyn_cast<StoreInst>(u)){
					Value * operand = si->getPointerOperand();
					if (find(used.begin(), used.end(), operand) == used.end()) used.push_back(operand);

				} else if (GetElementPtrInst *pi = dyn_cast<GetElementPtrInst>(u)){
					Value * operand = pi->getOperand(0);
					if (find(used.begin(), used.end(), operand) == used.end()) used.push_back(operand);
					...
					if (Function *f = dyn_cast<Function>(ci->getCalledOperand()->stripPointerCasts())){
						if (!f->isDeclaration()){
							unsigned arg_idx = 0;
							for (auto arg = f->arg_begin(); arg != f->arg_end(); ++arg){
								unsigned tid = arg->getType()->getTypeID();
								if (tid == llvm::ArrayType::PointerTyID ){
									Value * operand = ci->getArgOperand(arg_idx);
									if (find(used.begin(), used.end(), operand) == used.end()) used.push_back(operand);
								}
								arg_idx++;
							}
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  */
  ...
  /*MFUZZ*/
  GlobalVariable *AFLMemMapPtr =
      new GlobalVariable(M, PointerType::get(Int8Ty, 0), false,
                         GlobalValue::ExternalLinkage, 0, "__afl_mem_ptr");

  GlobalVariable *AFLMem2MapPtr =
      new GlobalVariable(M, PointerType::get(Int8Ty, 0), false,
                         GlobalValue::ExternalLinkage, 0, "__afl_mem2_ptr");

  FunctionCallee CheckLen = M.getOrInsertFunction("__check_len",Int64Ty,Type::getInt8PtrTy(C));
  FunctionCallee UpdateLen = M.getOrInsertFunction("__update_len",Type::getVoidTy(C),Int64Ty,Int32Ty);

//  FunctionCallee Debug_ = M.getOrInsertFunction("__debug",Type::getVoidTy(C),Int32Ty);
  ...
  /*MFuzz*/
    std::vector<Value*>taint = affectedVars[F.getName()];
	  taint_num +=taint.size();
	  std::vector<Value*>pointer = affectedPtrOps[F.getName()];
...
/* There is a problem with Ubuntu 18.04 and llvm 6.0 (see issue #63).
...
#ifdef AFL_HAVE_VECTOR_INTRINSICS
      if (ngram_size){
        PtrIdx = IRB.CreateZExt(
        IRB.CreateXor(PrevLocTrans, IRB.CreateZExt(CurLoc, Int32Ty)),Int32Ty);

        MapPtrIdx = IRB.CreateGEP(Int8Ty, MapPtr, PtrIdx);
      }
      else
#endif
      {
        PtrIdx = IRB.CreateXor(PrevLocTrans, CurLoc);
  ...
  if (!taint.empty())
	  {
		  for(auto &I :BB){
			  ...
			  if(find(taint.begin(), taint.end(), inst) != taint.end())
			  {
          ...
          if(StoreInst *si = dyn_cast<StoreInst>(&I)){
            ...
            if(isIntrinsic(ID)) {
              len = ci->getArgOperand(2);
            }
            else if(isStringFunc(name)){

              if (name.compare("strlen") == 0){

                len = dyn_cast<Value>(ci);

              }
              else {

                CallInst *checkLen = CallInst::Create(CheckLen,ci,"");
                checkLen->setMetadata(M.getMDKindID("nosanitize"),MDNode::get(C,None));
                checkLen->insertAfter(&I);
                len = dyn_cast<Value>(checkLen);
                insert = checkLen;
              }
            }

            CallInst *updateLen = CallInst::Create(UpdateLen, {len,PtrIdx},"");
            updateLen->setMetadata(M.getMDKindID("nosanitize"),MDNode::get(C,None));
            updateLen->insertAfter(insert);
            inst_mems++;

          }

				  if (ptr != NULL){
					  LoadInst *MemPtr = Builder.CreateLoad(AFLMemMapPtr);
					  MemPtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

					  LoadInst *Mem2Ptr = Builder.CreateLoad(AFLMem2MapPtr);
					  Mem2Ptr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

					  Value *MemCasted= Builder.CreatePtrToInt(ptr, Builder.getInt32Ty());
					  Value *MemTrunc = Builder.CreateAnd(MemCasted,ConstantInt::get(Int32Ty,0xffff));
					  Value *MemIdx = Builder.CreateXor(MemTrunc, PtrIdx);
					  Value *MemPtrIdx = Builder.CreateGEP(MemPtr, MemIdx);

					  LoadInst *MemCounter = Builder.CreateLoad(MemPtrIdx);
					  MemCounter->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
					  Value *MemIncr = Builder.CreateAdd(MemCounter, ConstantInt::get(Int8Ty, 1));
					  Builder.CreateStore(MemIncr, MemPtrIdx)
						  ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

					  Value *MemShr = Builder.CreateLShr(MemCasted, ConstantInt::get(Int32Ty,16));
					  Value *Mem2Idx= Builder.CreateXor(MemShr, PtrIdx);
					  Value *Mem2PtrIdx = Builder.CreateGEP(Mem2Ptr, Mem2Idx);

					  LoadInst *Mem2Counter = Builder.CreateLoad(Mem2PtrIdx);
					  Mem2Counter->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
					  Value *Mem2Incr = Builder.CreateAdd(Mem2Counter, ConstantInt::get(Int8Ty, 1));
					  Builder.CreateStore(Mem2Incr, Mem2PtrIdx)
						  ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
  ...
  if (!be_quiet) {
    ...
    if (!inst_blocks)
      ...
      OKF("Instrumented %d locations %u memories %u taints  (%s mode, ratio %u%%).", inst_blocks,
          inst_mems, taint_num, modeline, inst_ratio);
