...
static u8 __afl_mem_initial[MAP_SIZE];
static u8 __afl_mem2_initial[MAP_SIZE];

u8 * __afl_mem_ptr = __afl_mem_initial;
u8 * __afl_mem2_ptr = __afl_mem2_initial;
...
/*MFuzz*/
u64 __check_len (char * str){

  return strlen(str);
}

void __update_len (u64 len, int loc){

  u32 idx = (len ^ loc) & 0xffffffff;
  u32 mem_idx = 0xffff & idx;
  u32 mem2_idx = idx >>16;
  __afl_mem_ptr[mem_idx]++;
  __afl_mem2_ptr[mem2_idx]++;
//  return;
}
void __debug(int a){
...
static void __afl_map_shm(void) {
  ...
  /*MFuzz*/
  char *id_mem_str = getenv(SHM_MEM_VAR);
  char *id_mem2_str = getenv(SHM_MEM2_VAR);

  if (id_mem_str){
    u32 shm_id = atoi(id_mem_str);
    __afl_mem_ptr = shmat(shm_id,NULL, 0);
    if (__afl_mem_ptr == (void *)-1) _exit(1);
  }

  if (id_mem2_str){
    u32 shm_id = atoi(id_mem2_str);
    __afl_mem2_ptr = shmat(shm_id,NULL, 0);
    if (__afl_mem2_ptr == (void *)-1) _exit(1);
  }
...
int __afl_persistent_loop(unsigned int max_cnt) {
  ...
  if (first_pass) {
    ...
    /*MFuzz*/
    memset(__afl_mem_ptr, 0, MAP_SIZE);
    memset(__afl_mem2_ptr, 0, MAP_SIZE);
    ...
    __afl_area_ptr = __afl_area_ptr_dummy;
    __afl_mem_ptr = __afl_mem_initial;
    __afl_mem2_ptr = __afl_mem2_initial;
...
void __afl_manual_init(void) {
  ...
  if (getenv("AFL_DISABLE_LLVM_INSTRUMENTATION")) {
    ...
    if (__afl_mem_ptr == NULL) __afl_mem_ptr = __afl_mem_initial;
    if (__afl_mem2_ptr == NULL) __afl_mem2_ptr = __afl_mem2_initial;
